...
public class ColorPicker extends View {
     ...
     */
    // Listener method: http://stackoverflow.com/questions/3164716/user-defined-listener-in-java
    private PropertyChangeSupport support; //Listener Method *** Does it need to be "final"

    private final int paramOuterPadding = 2; // outer padding of the whole color picker view
    private final int paramInnerPadding = 7; // distance between value slider wheel and inner color wheel
    ...
    public ColorPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.support = new PropertyChangeSupport(this); // Listener
        init();
    }
    ...
    private void init() {
        ...
        colorPointerPaint = new Paint(); // More on the paint class can be found at http://developer.android.com/reference/android/graphics/Paint.html
        ...
        colorViewPath = new Path(); // More on the Path class can be found at http://developer.android.com/reference/android/graphics/Path.html
        valueSliderPath = new Path();
        arrowPointerPath = new Path();

        outerWheelRect = new RectF(); // More on the RectF class can be found at http://developer.android.com/reference/android/graphics/RectF.html
    ...
    /**
     * onMeasure: Measures the size of the screen so that it draws the right size color wheel
    ...
    /**
     * createColorWheel1Bitmap: Fills in the color wheel with all possible color gradients
     * @param width
     * @param height
     * @return
     */
    ...
    /**
     * onTouchEvent: This function handles the touches to the color wheel and saves the resulting
     *               color in a variable
     * @param event
     * @return
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (d <= colorWheelRadius) {
                ...
                invalidate();
                support.firePropertyChange("color",colorHSV,colorHSV); // Listener
    ...
    public void addPropertyChangeListener(PropertyChangeListener l){ // Listener
        support.addPropertyChangeListener(l);
    }

    public void addPropertyChangeListener(String propertyName, PropertyChangeListener l) { // Listener
    ...
    public void removePropertyChangeListener(PropertyChangeListener l) { // Listener
        support.removePropertyChangeListener(l);
    }

    public void removePropertyChangeListener(String propertyName, PropertyChangeListener l) { // Listener
        support.removePropertyChangeListener(propertyName, l);
    }


    /**
     * setColor: Allows the color to be set from an outside class
     * @param color
     */
    ...
    /**
     * getColor: Returns the master color to the calling function
     * @return
     */
    public int getColor() {
